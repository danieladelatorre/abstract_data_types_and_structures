...
//worst case Big O is O(n) for the partition function
int partition(SET *sp, void **copy, int low, int high){
	assert(sp!=NULL & copy!=NULL);
	int i = (low-1);
	void *temp;
	for(int j=low; j<=high-1; j++){
		if((*sp->compare)(copy[j], copy[high])<=0){
			i++;
			temp = copy[i];
			copy[i] = copy[j];
			copy[j] = temp;
...
//worst case Big O is O(n^2) including the partition function
void quickSort(SET *sp, void**copy, int low, int high){
	assert(copy!=NULL);
	if(low<high){
		int part = partition(sp, copy, low, high);
		quickSort(sp, copy, low, part-1);
		quickSort(sp, copy, part+1, high);
	}
	return;
}

//worst case Big O is O(n^2) including the quickSort function and O(n) not including the other functions
